"use client";

import Link from "next/link";
import Image from "next/image";

const navLinks = [
  { href: "/menu", label: "Menu" },
  { href: "/about", label: "Our Story" },
  { href: "/private-events", label: "Private Events" },
  { href: "/careers", label: "Careers" },
  { href: "/reserve", label: "Reserve" },
];

export default function Footer() {
  return (
    <footer className="bg-nina-navy text-nina-cream">
      {/* Meander border */}
      <div
        style={{
          background: "url(/assets/meander-cream.png) repeat-x top",
          backgroundSize: "auto 44px",
          height: "44px",
          opacity: 0.25,
        }}
      />

      <div className="container-narrow py-16 md:py-20">
        {/* Center: Medallion + Wordmark */}
        <div className="flex flex-col items-center text-center mb-12">
          <Image
            src="/assets/mark-cream.png"
            alt="Nina The Greek medallion"
            width={48}
            height={48}
            className="mb-4"
          />
          <Link
            href="/"
            className="font-serif text-2xl md:text-3xl tracking-wide text-nina-cream transition-opacity duration-200 hover:opacity-80"
          >
            nina the greek
          </Link>
        </div>

        {/* Nav links */}
        <nav className="flex flex-wrap justify-center gap-6 md:gap-8 mb-12">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="transition-colors duration-200"
              style={{
                fontFamily: "var(--font-sans)",
                fontWeight: 500,
                fontSize: "11px",
                letterSpacing: "0.18em",
                textTransform: "uppercase" as const,
                color: "#faf8f5",
              }}
              onMouseEnter={(e) => {
                (e.target as HTMLElement).style.color = "#9bb6dd";
              }}
              onMouseLeave={(e) => {
                (e.target as HTMLElement).style.color = "#faf8f5";
              }}
            >
              {link.label}
            </Link>
          ))}
        </nav>

        {/* Info grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center mb-12">
          {/* Address */}
          <div>
            <p
              className="mb-2"
              style={{
                fontFamily: "var(--font-sans)",
                fontSize: "11px",
                letterSpacing: "0.3em",
                textTransform: "uppercase" as const,
                color: "#9bb6dd",
              }}
            >
              Location
            </p>
            <address className="not-italic font-serif text-lg leading-relaxed text-nina-cream">
              <a
                href="https://www.google.com/maps/search/?api=1&query=Nina%20The%20Greek%2C%20Avenue%2057%2C%20Maqabah%2C%20Saar%2C%20Bahrain"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:text-nina-muted transition-colors"
              >
                Avenue 57, Maqabah
                <br />
                Saar, Bahrain
              </a>
            </address>
          </div>

          {/* Hours */}
          <div>
            <p
              className="mb-2"
              style={{
                fontFamily: "var(--font-sans)",
                fontSize: "11px",
                letterSpacing: "0.3em",
                textTransform: "uppercase" as const,
                color: "#9bb6dd",
              }}
            >
              Hours
            </p>
            <p className="font-serif text-lg leading-relaxed text-nina-cream">
              Open Daily
              <br />
              3:00 PM &ndash; 11:00 PM
            </p>
          </div>

          {/* Contact + Social */}
          <div>
            <p
              className="mb-2"
              style={{
                fontFamily: "var(--font-sans)",
                fontSize: "11px",
                letterSpacing: "0.3em",
                textTransform: "uppercase" as const,
                color: "#9bb6dd",
              }}
            >
              Contact
            </p>
            <p className="font-serif text-lg text-nina-cream mb-2">
              <a
                href="tel:+97317322777"
                className="transition-opacity duration-200 hover:opacity-80"
              >
                1732 2777
              </a>
            </p>
            <a
              href="https://www.instagram.com/nina_the_greek/"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 transition-opacity duration-200 hover:opacity-80"
              style={{
                fontFamily: "var(--font-sans)",
                fontSize: "12px",
                letterSpacing: "0.15em",
                color: "#9bb6dd",
              }}
            >
              <svg
                className="w-4 h-4"
                fill="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              >
                <path
                  fillRule="evenodd"
                  d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z"
                  clipRule="evenodd"
                />
              </svg>
              @nina_the_greek
            </a>
          </div>
        </div>

        {/* Divider + copyright */}
        <div
          className="pt-8 text-center"
          style={{ borderTop: "1px solid rgba(250,248,245,.15)" }}
        >
          <p
            className="text-xs"
            style={{ color: "#9bb6dd", lineHeight: 1.7 }}
          >
            &copy; {new Date().getFullYear()} Nina The Greek. All rights
            reserved.
          </p>
          <p
            className="mt-1 text-xs"
            style={{ color: "#9bb6dd", opacity: 0.7 }}
          >
            Prices inclusive of 10% VAT &amp; 5% government levy
          </p>
        </div>
      </div>
    </footer>
  );
}
